...
HedgesD <- function (x) {
  ...
  J_a <- 1 - (3 / (4 * (N_t_a + N_c_a) - 9))  
  d.1a <- (X_t_a - X_c_a)/S.tot_a 
  d.2a <- d.1a * J_a
  
  v.1a <- (1 / N_t_a) + (1 / N_c_a)
  v.2a <- d.2a^2 / (2 * (N_t_a + N_c_a))
  v.3a <- v.1a + v.2a
  ...
  J_b <- 1 - (3 / (4 * (N_t_b + N_c_b) - 9))  
  d.1b <- (X_t_b - X_c_b)/S.tot_b 
  d.2b <- d.1b * J_b
  
  v.1b <- (1 / N_t_b) + (1 / N_c_b)
  v.2b <- d.2b^2 / (2 * (N_t_b + N_c_b))
  v.3b <- v.1b + v.2b
  
  N_c <- N_c_a + N_c_b
  cov.d1 <- (d.2a * d.2b) / (2 * (N_c + N_t_a + N_t_b))
  cov.d <- cov.d1 + (1 / N_c)
